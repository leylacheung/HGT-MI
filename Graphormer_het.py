...
def shortest_path_distance(data: Data) -> Tuple[Dict[int, List[int]], Dict[int, List[int]]]:
    ...
    G = edge_index_to_networkx(data[('ring', 'r2r', 'ring')].edge_index.detach().cpu())
...
def batched_shortest_path_distance(data) -> Tuple[Dict[int, List[int]], Dict[int, List[int]]]:
    graphs = [edge_index_to_networkx(sub_data[('ring', 'r2r', 'ring')].edge_index.detach().cpu()) for sub_data in data.to_data_list()]
    relabeled_graphs = []
    shift = 0
...
class Graphormer(nn.Module):
    def __init__(self,
        ...
        """
        :param num_layers: number of Graphormer layers，Graphormer 
