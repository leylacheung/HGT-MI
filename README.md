...
- **CEPDB**  
  Sourced from Harvard’s Clean Energy Project, originally ~2.3 M potential donor molecules.  
  After RDKit validation, 2 225 974 valid SMILES remain, each with DFT-computed PCE, HOMO, LUMO, band gap, Voc and Jsc—ideal for large-scale pre-training.

- **HOPV**  
  The Harvard Organic Photovoltaic dataset: 350 small molecules and polymers with *experimental* PCE measurements.  
  Although small, it is a classic benchmark for testing real-world generalisation.

- **PFD**  
  1 055 polymer donors from earlier polymer–fullerene devices (Nagasawa *et al.*).  
  Duplicate nicknames were merged and the highest recorded PCE retained, reducing noise in the labels.

- **NFA**  
  654 unique non-fullerene acceptor (NFA) molecules extracted from the Polymer:NFA set.  
  Samples sharing the same acceptor were merged and the best experimental PCE kept; emphasises modern NFA chemistry.

- **PD**  
  A donor-centric slice of Polymer:NFA: 277 polymer donors.  
  Duplicate donors were consolidated with their highest PCE value, providing a focused view on donor performance.

- **DA_Pair 51K**  
  ~51 000 virtual donor-acceptor pairs from Lopez *et al.*; donor fixed to PCDTBT, acceptors generated by combinatorial assembly of 107 fragments.  
  HOMO/LUMO/band gap via DFT and PCE estimated by a modified Scharber model; pairs with negative or extremely low PCE were removed.

- **DA_Pair 1.2K**  
  1 200 literature-reported donor–acceptor pairs with *experimental* PCE.  
  After de-duplication and outlier filtering, it serves as a realistic benchmark for modelling intermolecular interactions in OSC devices.
